 ...
 *       interceptor converts it transparently to the same magic-prefixed {@code B} marker. Use
 *       {@link #isFloat32Vector(AttributeValue)} to identify it and {@link
 *       #toFloats(AttributeValue)} to access its values. Passing a returned marker back to a write
 *       preserves the compact {@code FLOAT32VECTOR} storage type.
 * </ul>
 *
...
public final class Float32Vector {
   ...
   * Binary ({@code B}) attribute whose bytes start with the Float32Vector magic prefix and whose
   * remaining payload contains a whole number of 32-bit floats.
   *
