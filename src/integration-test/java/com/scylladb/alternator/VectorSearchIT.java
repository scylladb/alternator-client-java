...
public class VectorSearchIT {
  ...
  private void assumeVectorStoreEnabled() {
    assumeVectorStoreEnabled(tableName, "vi1");
  }

  private void assumeVectorStoreEnabled(String targetTableName, String indexName) {
    ...
    VectorSearchSupport.DescribeTableWithVectorIndexes desc =
        VectorSearchSupport.describeTable(
            client, DescribeTableRequest.builder().tableName(targetTableName).build());
    assumeTrue(
  ...
  private void waitForVectorIndexActive(String indexName) throws InterruptedException {
    for (int i = 0; i < 120; i++) {
      ...
      boolean indexActive =
          desc.vectorIndexes().stream()
              .anyMatch(
                  vi -> indexName.equals(vi.indexName()) && "ACTIVE".equals(vi.indexStatus()));
      if (indexActive) {
        return;
      }
      TimeUnit.MILLISECONDS.sleep(500);
    ...
    fail(
        "Timed out waiting for vector index '"
            + indexName
            + "' to become ACTIVE on table '"
            + tableName
            + "'");
  }

  private VectorSearchSupport.CreateTableWithVectorIndexes createTableWithVectorIndex() {
    VectorIndex vi =
        VectorIndex.builder()
            .indexName("vi1")
            .vectorAttribute(
    ...
    return createTableWithVectorIndex(base, Collections.singletonList(vi));
  }

  private VectorSearchSupport.CreateTableWithVectorIndexes createTableWithVectorIndex(
      CreateTableRequest base, List<VectorIndex> vectorIndexes) {
    try {
      return VectorSearchSupport.createTable(client, base, vectorIndexes);
    } catch (DynamoDbException e) {
      skipIfVectorFeatureUnsupported(e);
      throw e;
    }
  }

  private static void skipIfVectorFeatureUnsupported(DynamoDbException e) {
    if (isVectorFeatureUnsupported(e)) {
      assumeTrue("Skipping: server does not support vector indexes: " + e.getMessage(), false);
    }
  }

  private static boolean isVectorFeatureUnsupported(DynamoDbException e) {
    String message = exceptionText(e);
    if (message.contains("vector store is disabled")) {
      return true;
    }
    if ((message.contains("unknown") || message.contains("unrecognized"))
        && message.contains("vector")) {
      return true;
    }
    return (message.contains("vectorindexes")
            || message.contains("vector indexes")
            || message.contains("vector store"))
        && (message.contains("unsupported")
            || message.contains("not supported")
            || message.contains("disabled"));
  }

  private static String exceptionText(DynamoDbException e) {
    StringBuilder text = new StringBuilder();
    if (e.awsErrorDetails() != null) {
      if (e.awsErrorDetails().errorCode() != null) {
        text.append(e.awsErrorDetails().errorCode()).append(' ');
      }
      if (e.awsErrorDetails().errorMessage() != null) {
        text.append(e.awsErrorDetails().errorMessage()).append(' ');
      }
    }
  ...
  public void testCreateTableWithVectorIndex() throws Exception {
    VectorSearchSupport.CreateTableWithVectorIndexes result = createTableWithVectorIndex();
    ...
    assertEquals(tableName, result.response().tableDescription().tableName());
    assertEquals(1, result.vectorIndexes().size());
    assertEquals("vi1", result.vectorIndexes().get(0).indexName());
  }
  ...
  public void testQueryWithReturnScores() throws Exception {
    ...
    if (!result.items().isEmpty()) {
      assertFalse(
          "Expected scores to be returned when ReturnScores=true", result.scores().isEmpty());
  ...
  public void testValidSimilarityFunctions() throws Exception {
    ...
    for (String sf : Arrays.asList("COSINE", "DOT_PRODUCT", "EUCLIDEAN")) {
      ...
      try {
        ...
        createTableWithVectorIndex(base, Collections.singletonList(vi));
        assumeVectorStoreEnabled(tbl, "vi-" + sf.toLowerCase());
      } finally {
        try {
