...
public class VectorSearchInterceptorTest {
  ...
  @Test
  public void testWithVectorIndexUpdatesRequiresExactlyOneUpdate() {
    VectorIndexUpdate update =
        VectorIndexUpdate.builder()
            .delete(DeleteVectorIndexAction.builder().indexName("old-idx").build())
            .build();
    UpdateTableRequest request = UpdateTableRequest.builder().tableName("items").build();

    try {
      VectorSearchSupport.withVectorIndexUpdates(request, Collections.emptyList());
      fail("empty vector index update list should be rejected");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("exactly one"));
    }

    try {
      VectorSearchSupport.withVectorIndexUpdates(request, Arrays.asList(update, update));
      fail("multiple vector index updates should be rejected");
    } catch (IllegalArgumentException expected) {
      assertTrue(expected.getMessage().contains("exactly one"));
    }
  }
  ...
  public void testVectorQueryResultSnapshotsScores() {
    software.amazon.awssdk.services.dynamodb.model.QueryResponse response =
        software.amazon.awssdk.services.dynamodb.model.QueryResponse.builder().build();
    List<Double> scores = new ArrayList<>(Arrays.asList(0.9, 0.8));
    VectorQueryResult result = new VectorQueryResult(response, scores);
  ...
  @Test
  public void testAttributeValueToJsonMagicPrefixedOrdinaryBinaryIsBase64() throws Exception {
    AttributeValue marker = Float32Vector.toAttributeValue(0.5f, -1.5f);
    AttributeValue ordinaryBinary = AttributeValue.fromB(marker.b());
    ObjectNode json = VectorSearchInterceptor.attributeValueToJson(ordinaryBinary);
  ...
  // -------------------------------------------------------------------------
  // Float32Vector — marker encoding used by request and response replacement
  // -------------------------------------------------------------------------
  ...
  public void testFloat32VectorFromListRoundTrip() {
    // Converting an ordinary L-typed vector via toAttributeValue(List) must produce
    // the same marker as toAttributeValue(float...) with the original values.
    float[] original = {1.5f, -2.5f, 0.0f};
    AttributeValue fromFloats = Float32Vector.toAttributeValue(original);

    // Build the numeric elements of an ordinary L-typed AttributeValue.
