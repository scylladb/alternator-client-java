...
public class VectorSearchHttpInterceptorTest {
  ...
  public void testUnchangedUncompressedHttpResponsePreservesChecksumAndLengthHeaders()
    ...
    assertTrue(modified.isPresent());
    assertSame(headerContext.responseBody().get(), modified.get());
  ...
  public void testUnchangedUncompressedAsyncHttpResponsePreservesChecksumAndLengthHeaders()
    ...
    Publisher<ByteBuffer> originalPublisher = singleChunkPublisher(responseBody);
    Context.ModifyHttpResponse headerContext =
        responseContext(listTablesHttpRequest(), response, originalPublisher);
    SdkHttpResponse modifiedResponse =
        VectorSearchInterceptor.INSTANCE.modifyHttpResponse(headerContext, attrs);
    ...
    assertTrue(modified.isPresent());
    assertSame(originalPublisher, modified.get());
  ...
  @Test
  public void testNonCrc32ChecksumMismatchesAreRetryable() {
    byte[] responseBody = bytes("{\"Item\":{\"id\":{\"S\":\"item-1\"}}}");
    String[][] mismatchedChecksums = {
      {"x-amz-crc32c", "0"},
      {"x-amz-checksum-crc32c", "invalid"},
      {"x-amz-checksum-sha1", "invalid"},
      {"x-amz-checksum-sha256", "invalid"},
      {"x-amz-checksum-crc64nvme", "invalid"}
    };

    for (String[] mismatch : mismatchedChecksums) {
      SdkHttpResponse response =
          SdkHttpResponse.builder().statusCode(200).putHeader(mismatch[0], mismatch[1]).build();
      ...
      assertTrue(failure.getMessage().contains(mismatch[0]));
      assertTrue(failure.getMessage().contains("different checksum"));
    }
  }
  ...
  public void testSdkSyncClientRetriesInterceptorResponseReadFailure() {
    ...
    try {
      ...
      assertEquals("item-1", response.item().get("id").s());
  ...
  public void testAsyncResponseCancellationStopsBufferingAndDownstreamSignals() {
    ...
    Optional<Publisher<ByteBuffer>> modified =
        VectorSearchInterceptor.INSTANCE.modifyAsyncHttpResponseContent(
            responseContext(getItemHttpRequest(), source), new ExecutionAttributes());
  ...
  public void testAsyncResponseCancellationFromOnNextSuppressesCompletion() {
    ...
    Optional<Publisher<ByteBuffer>> modified =
        VectorSearchInterceptor.INSTANCE.modifyAsyncHttpResponseContent(
            responseContext(
                getItemHttpRequest(), singleChunkPublisher(bytes("{\"TableNames\":[\"items\"]}"))),
            new ExecutionAttributes());
  ...
  private static void assertFloat32Vector(JsonNode attribute, float... expected) {
    ...
    assertNotNull(attribute.get("NULL"));
    assertFalse(attribute.get("NULL").booleanValue());
    AttributeValue marker =
        AttributeValue.builder()
            .b(SdkBytes.fromByteArray(Base64.getDecoder().decode(attribute.get("B").asText())))
            .nul(false)
            .build();
      ...
      return requestBodyFuture.thenAccept(
          ignored -> {
            ...
            if (attempt == 1) {
              ...
              responseBody = bytes("{\"Item\":{\"id\":{\"S\":\"item-1\"}}}");
